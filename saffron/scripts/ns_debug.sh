...
addr2line() {
  # Use Docker with devkitpro image for cross-platform addr2line
  docker run --rm -v "$(pwd):/work" devkitpro/devkita64 \
    /opt/devkitpro/devkitA64/bin/aarch64-none-elf-addr2line -e "/work/$elf_path" -f -p -C -a "$1"
}
