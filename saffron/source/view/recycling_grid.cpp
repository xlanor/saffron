...
RecyclingGridItem::RecyclingGridItem() {
    ...
    this->registerClickAction([this](View*) {
        brls::Box* view = dynamic_cast<brls::Box*>(this->getParent()->getParent());
...
RecyclingGrid::RecyclingGrid() {
    ...
    this->registerFloatXMLAttribute("spanCount", [this](float value) {
        ...
        this->spanCount = static_cast<int>(value);
        this->reloadData();
    });
    ...
    this->registerFloatXMLAttribute("preFetchLine", [this](float value) {
        this->preFetchLine = static_cast<int>(value);
        this->reloadData();
    });
...
void RecyclingGrid::draw(
    ...
    if (!this->dataSource || this->dataSource->getItemCount() == 0) {
        ...
        float w2 = hintLabel->getWidth();
        float h2 = hintLabel->getHeight();
        this->hintLabel->setAlpha(this->getAlpha());
        this->hintLabel->draw(vg, x + (width - w2) / 2, y + height / 2, w2, h2, style, ctx);
    }
}
...
void RecyclingGrid::notifyDataChanged() {
    ...
    if (dataSource) {
        if (dataSource->getItemCount() > 0 && contentBox->getChildren().empty()) {
            reloadData();
            return;
        }
...
void RecyclingGrid::cancelAllPendingImages() {
...
brls::View* RecyclingGrid::getNextCellFocus(brls::FocusDirection direction, brls::View* currentView) {
    ...
    if (direction == brls::FocusDirection::LEFT && (currentIndex % spanCount) == 0) {
        View* next = getParentNavigationDecision(this, nullptr, direction);
        if (!next && hasParent()) next = getParent()->getNextFocus(direction, this);
        return next;
    }

    if (direction == brls::FocusDirection::RIGHT && ((currentIndex % spanCount) == (size_t)(spanCount - 1) ||
        currentIndex == this->dataSource->getItemCount() - 1)) {
        View* next = getParentNavigationDecision(this, nullptr, direction);
        if (!next && hasParent()) next = getParent()->getNextFocus(direction, this);
        return next;
    }
    ...
    size_t currentFocusIndex = currentIndex + offset;
